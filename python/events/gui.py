...
def handle_x_button_click(card: ui.components.card.Card, database: schif.Parol) -> None:
    ...
    polutils.patterns.event.post("remove_password", card.title)

def setup_gui_event_handlers(database: schif.Parol) -> None:
    polutils.patterns.event.subscribe("new_card", lambda args: handle_new_card(*args))
    polutils.patterns.event.subscribe("remove_card", lambda card: handle_x_button_click(card, database))